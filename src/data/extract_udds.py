...
SOURCE_FILE = PROJECT_ROOT / "data" / "raw" / "UDDS_raw.txt"
OUTPUT_FILE = PROJECT_ROOT / "data" / "raw" / "UDDS_raw.csv"
...
# The EPA file contains two metadata/header lines.
# Actual data begins on line 3.
df_raw = pd.read_csv(
    SOURCE_FILE,
    sep=r"\s+",
    skiprows=2,
...
# ---------------------------------------------------------
# Clean possible non-numeric rows
# ---------------------------------------------------------
...
# Convert mph → km/h
# ---------------------------------------------------------
velocity_kmh = df_raw["velocity_mph"].to_numpy() * 1.609344

time_s = df_raw["time_s"].to_numpy()

# No road-grade information in standard UDDS
